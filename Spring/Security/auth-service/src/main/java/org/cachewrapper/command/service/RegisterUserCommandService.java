...
public class RegisterUserCommandService implements CommandService<JsonWebTokenResponse, RegisterUserCommand> {
    ...
    public JsonWebTokenResponse execute(RegisterUserCommand command) {
        ...
        if (userCredentialsService.findByEmail(email) != null) {
            throw new EmailAlreadyExistsException(email);
        }

        if (userCredentialsService.findByUsername(username) != null) {
            throw new UsernameAlreadyExistsException(username);
        }

        final String passwordHash = Objects.requireNonNull(passwordEncoder.encode(password));
        final var userCredentialsAggregate = new UserCredentialsAggregate(email, username, passwordHash);
    ...
    @NotNull
    // TODO: Переписать и сделать общий token create response и инкапсулировать метод
